...
브라우저에서 바로 쓰는 정적 웹앱입니다. 한글 텍스트를 붙여 넣고 GPT, Gemini, Claude 중 하나를 선택한 뒤 API 키를 입력하면 윤문본을 받아와 복사할 수 있습니다.
...
API 키는 브라우저에서 선택한 AI 제공자 API 호출에만 사용됩니다. `이 브라우저에 API 키 저장`을 켜면 제공자별로 `localStorage`에 저장되므로, 공용 PC에서는 사용하지 않는 편이 좋습니다.
